...
We develop InTraVisTo\footnote{\rlap{\url{https://github.com/daviderigamonti/InTraVisTo}}} as a tool designed to visualize internal states and the information flow within Transformer-based neural networks underlying modern LLMs.
...
As we can observe from~\cref{fig:exp_fom_1_B1}, the set similarity metrics for the Llama 2 FOM suggest a certain affinity to the corresponding Markov model.
...
This appears to be true even for FOMs based on models such as Mistral, which despite showing a slight tendency to as it is possible to observe by comparing \cref{table:exp_fom_llama-kl} and~\cref{table:exp_fom_mistral-kl}.
Another aspect that can be deducted from \cref{table:exp_fom_llama-kl,table:exp_fom_mistral-kl} is the unsuccessful application of the RMS normalization, as it exemplifies how the performance of FOM models with RMS falls below to that of their standard counterparts across the majority of tasks.
...
We explored aspects of Transformer interpretability, focusing on the creation of a specialized tool to visualize internal states of Transformer-based language models: \textbf{InTraVisTo}.
