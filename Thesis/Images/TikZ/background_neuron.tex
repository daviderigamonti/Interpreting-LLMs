...
\begin{tikzpicture}
    ...
    \node[below = 2em of in3, helv, text=black!35!green, align=center] () {\Large Inputs};
    \node[below = 2em of out, helv, text=black!30!cyan, align=center] () {\Large Output};
    \node[below = 2em of neuron, helv, text=black, align=center] () {\Large Neuron};
    \node[right = 1.5em of bias, helv, text=black!30!gray] () {\Large Bias Term};


    \node[above left = 8em and 1em of out](actfn){
        \begin{axis}[scale=0.25,axis lines=middle,xtick=\empty, ytick=\empty]
            \addplot[violet, domain=-1:1, smooth]{tanh(x)};
        \end{axis}
    };

    \node[above left = 0em and -4em of actfn, helv, text=black!10!violet, align=center] () {\large Activation \tikznewl{\large Function $f(\cdot)$}};
\end{tikzpicture}
