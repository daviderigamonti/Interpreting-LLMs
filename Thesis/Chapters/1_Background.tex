...
\Cref{fig:background_rnn} shows the unrolling process of a RNN, visualizing how the recurrent connection contributes to the distributed hidden state at each time instant.
...
\todo[purple!30]{Write paragraph for attention dimensional analysis, plus image}
...
In the case of Llama models~\cite{touvron2023,dubey2024}, the gating mechanism happens through an element-wise multiplication where $\gbm{W}_{in} \in \mathbb{R}^{D \times d_{hid}}$ represents the input transformation matrix with its respective bias $\gbm{b}_{in}$, $\gbm{V} \in \mathbb{R}^{D \times d_{hid}}$ is the gate transformation matrix with its respective bias $\gbm{b_{V}}$, $\gbm{W}_{out} \in \mathbb{R}^{d_{hid} \times D}$ performs the out-projection to the model's hidden representation dimensionality with its respective bias $\gbm{b}_{out}$ and $f$ represents the activation/gating function (in the SwiGLU case $f(\gbm{x}) = Swish(\gbm{x})$):
\begin{equation}
    \label{eq:background_ffnn}
    FFN(\gbm{x}) = GLU(\gbm{x}) \cdot \gbm{W}_{out} + \gbm{b}_{out} 
    = \Bigl( f(\gbm{x}\gbm{V} + \gbm{b_V}) \odot \gbm{x}\gbm{W}_{in} + \gbm{b}_{in} \Bigr) \cdot \gbm{W}_{out} + \gbm{b}_{out}
\end{equation}
