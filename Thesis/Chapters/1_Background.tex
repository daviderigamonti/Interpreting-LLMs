With the current escalation in popularity and pervasiveness of machine learning solutions applied to the realm of natural languages, there exists an increasingly relevant topic that is still being the subject of extensive studies by experts in this field: \emph{interpretabilty}.
...
Considering the introduction of \emph{deep learning}, the need for interpretabilty spiked as models got increasingly more complex.
...
In particular, we are interested in a specific class of machine learning models that are currently considered state-of-the art in the Natural Language Processing (NLP) field: \emph{Large Language Models (or LLMs)}.
...
Particular focus will be placed on the observation and interpretation of the models' hidden states, which are their internal representations, conveying condensed information between various internal components and acting as the \emph{internal language} of the language model.

More specifically, we propose \emph{InTraVisTo}: an interactive tool designed to support and aid machine learning researchers in hypothesis formulation by offering a unique perspective on the internal representations of large language models.
...
Machine Learning is commonly considered a subfield of Artificial Intelligence, and is characterized by the idea of training a machine to learn from \emph{past experience}, without providing an explicit algorithm to execute.
The term ``past experience'' is often used loosely in this scenario as it is not necessarily tied with information directly perceived by an agent within a specific environment.
...
There exist three main \emph{learning paradigms} for machine learning (see~\cref{fig:background_ml-paradigm}):
\begin{itemize}
...
\emph{Neural networks} are one of the most popular machine learning techniques.
...
Before transmitting their output, they also apply an activation function which in most cases consists of a \emph{S-shaped} nonlinear function.
...
The central process used by neural networks to learn new information is \emph{backpropagation}.
...
The gradient is said to be ``accumulated inside neurons'' for computational reasons, however it is always computed starting from a loss function at the output of the network and with respect to the parameters of the network.

\subsection{Deep Learning}

\emph{Deep Learning} is a further specialization of machine learning.
...
The most important property obtained through the dimensional scale-up is the hierarchical \emph{feature extraction} capability, which consists in obtaining rich condensed representations that follow a semantic hierarchy from the latent spaces defined by the network's layer projections.
...
However, the sheer scale of deep neural networks makes the leveraging of these condensed representations a viable option to build an artificial feature base that has many upsides with respect to human-defined features.
For instance, we may require expert knowledge in order to craft an appropriate set of meaningful features from a raw dataset, while in a deep learning scenario, the model would be able to automatically create features for itself.
...
The main drawback of the deep learning approach comes from the fact that learned features are not directly \emph{human-understandable}, as they are the result of mathematical optimization.
...
The set of compressed features is commonly referred to as \emph{latent feature space} or \emph{embedding space}, due to the inherent impossibility of directly interpreting its variables.
...
Another type of deep architecture relevant for this work is the \emph{Recurrent Neural Network (RNN)} architecture: one of the first promising approaches to text generation which featured a structure much more similar to classic neural networks, but with the addition of \emph{recurrent connections}.
...
\cref{fig:background_rnn} shows the \emph{unrolling} process of an RNN, visualizing how the recurrent connection contributes to the distributed hidden state at each time instant.
...
The most popular architecture for text generation before the current state-of-the-art models was the \emph{Long Short-Term Memory (LSTM)}.
...
The significant vanishing gradient in RNNs is a direct consequence of the co-occurrence between activation functions without a prominent zero-valued region and the \emph{backpropagation through time} process (being the backpropagation equivalent of a network using recurrent connections).
...
LSTMs solve this issue by applying specialized gating functions (highlighted in~\cref{fig:background_lstm}) that allow the model to decide what information to remember, forget and pass along into what is called the \emph{constant error carousel}.
...
The main focus of most state-of-the art models is text generation, even though it is possible to repurpose them for many other tasks due to their modular nature and natural language outputs.
...
Exactly in these years the use of \emph{Markov models} began to rise in popularity, despite having been theorized decades earlier~\cite{shannon1948}.
...
In fact, Markov models are a broader categorization of stochastic models characterized by the \emph{Markov property}, stating that transitions to future states must depend solely on the current state, without considering prior states or transitions.
...
A Markov model is said to be of order $n-1$ when it operates with $n$-grams; therefore, a Markov model of order $0$ directly models vocabulary frequency with fixed probabilities and a Markov model of order $1$ models a vocabulary distribution for the next word given any unigram representing the previous word.
...
Following the concept of stochastic text analysis, it is also possible to represent an entire document as an unordered set of $n$-gram frequencies, this approach is known as \emph{Bag of Words (BoW) model}.
...
This type of representation is not limited to direct text frequency as a metric to represent word prevalence within a document, since specialized metrics such as \emph{TF-IDF} are also used to provide more accurate estimates for information retrieval and document classification tasks.
...
To address this limitation, more advanced techniques, such as word embeddings, were created.

\subsection{Word Embeddings}

One of the most relevant and groundbreaking developments in the field of NLP was the introduction of \emph{word embeddings}.
...
The main appeal of word embeddings is their dimensionality reduction effect, since the original vocabulary representation (bag of words) suffers from the \emph{curse of dimensionality} to a great extent.
...
However, the first successful implementation of word embeddings can be attributed to \citet{mikolov2013} with \emph{word2vec}, greatly inspired by Bengio's previous work.
...
The context is fed to the model as a symmetric window containing all terms occurring around the target term, modeling a many-to-1 prediction (\cref{fig:background_word2vec-cbow}).
...
\end{itemize}
The results are generally comparable between the two techniques, with the key aspect being that the full context of each word is integrated into the word representation itself.
...
However, in the case of word embeddings, these arithmetic properties have been reported to be significantly much more prominent and also reflective of actual word semantics.
...
One of the most famous examples presented in~\cite{mikolov2013} is: \texttt{emb(`King') - emb(`Man') + emb(`Woman') $\simeq$ emb(`Queen')}, which models both the relationships of \emph{gender} and \emph{royalty}. % chktex 36
...
In principle, any kind of semantic relationship can potentially be modeled by word embeddings given sufficient support in the training dataset.
...
Understanding the semantic associations underlying text models can provide an educated perspective on how these models interpret their inputs, but also underline human and language biases.
In addition, given the focus on interpretability of this work, this topic is particularly relevant to many of the proposed experiments.

\section{Transformer Architectures}

\emph{Transformers} are the current state-of-the art model architecture for most text applications and not only.
Although they were initially conceived for processing textual inputs of sequential nature, in recent years they saw discrete success in a wide variety of other applications (in particular image processing), albeit occasionally with some slight variations in form.
...
The first Transformer architecture was introduced by~\citet{vaswani2017} in the seminal 2017 paper \emph{``Attention Is All You Need''}.
This architecture, shown in~\cref{fig:background_trans-enc-dec}, is of the \emph{encoder-decoder} type and follows the typical paradigm of \emph{sequence-to-sequence (seq2seq)} learning, implying the existence of two separate modules (an encoder and a decoder) which work in tandem to elaborate inputs and provide an output, while both input and output consist of sequential data.
...
The encoder and the decoder have a fairly similar structure, both featuring multiple basic blocks that can be sequentially connected to each other (operation commonly referred to as \emph{stacking}).
...
\begin{itemize}
    ...
    \item Finally, the \textbf{postprocessing step}, which covers the step that concludes the generation process of the Transformer architecture.
\end{itemize}
...
These additional improvements are the subject of further discussion in a dedicated section (\cref{ssec:background_transf_current}).

\subsubsection*{Preprocessing}

The first step to process inputs is \emph{tokenization}, as it is fundamental to reduce the possibly infinite vocabulary to a finite set of tokens that can be embedded and understood by the model.
This is normally done by a tokenizer, which performs some preprocessing on the input sentence to obtain a sequence of tokens that is then fed to the model.

Most Transformer architectures use different variations of \emph{sub-word tokenization} algorithms, meaning that the tokenizer does not parse the text word-by-word, but token-by-token.
A token is considered the fundamental vocabulary object, without a fixed correlation to words, as each word can correspond to a combination of multiple tokens and vice versa.
Sub-word tokenization is usually obtained as the result of a training process where common character sequences get progressively aggregated until they reach a certain frequency threshold, at this point a compact and efficient representation of the vocabulary that is balanced between granularity and size is achieved.
This type of tokenization improves the ability of the model to deal with rare and unknown words, greatly limiting the use of \emph{unknown tokens} (special tokens used in place of certain words that cannot be correctly tokenized and encoded) and potentially facilitating the creation of multilingual models.
...
Additionally, the Transformer architecture is \emph{word position agnostic}, meaning that embedded tokens do not directly contain any additional information indicating their position within the input sentence to the model.
This undesirable feature is circumvented by implementing \emph{positional encoding}: an additive mask that gets applied to the input tokens right after the embedding step.
...
In its most basic form, attention can simply be defined as the operation of weighting each element of an input sequence according to its relevance to the other elements belonging to that sequence.
...
The most important aspect of attention is its scoring function, which allows the decoder to \emph{focus} on certain tokens by modeling a weight distribution over the attended hidden states using an attention function.
...
The basic Transformer architecture implements three different types of attention:
\begin{itemize}
    \item An \textbf{encoder-decoder attention (or cross-attention)}, which is found inside the decoder block, and attends to the encoder blocks' hidden states using its current state as the \emph{query} (\cref{fig:background_enc-dec-attention}).
    \item The \textbf{self-attention}, which is used by both the encoder block to process the hidden states at its input.
It works by following the same principles of cross-attention; however, it attends to representations of the same nature, meaning that \emph{query}, \emph{key} and \emph{value} are extrapolated by the same set of hidden states.
In practice, the model is able to manage long-range dependencies and possibly capture patterns or associations between representations in a completely parallelizable way.
    \item Lastly, \textbf{masked self-attention} is a specific implementation of self-attention that can be found in the decoder block, due to its need to perform autoregressive modeling (\cref{fig:background_self-attention}).
...
In reality, Transformers use a more refined attention implementation called \emph{multi-head attention}, which can be applied to all the previously identified types of attentions.
...
Due to the replication of attention vectors, this operation influences their dimensionality by a factor of $\frac{1}{H}$, where $H$ is the chosen number of separate attention heads for multi-head attention.
...
As the name suggests, it consists of a simple feed-forward network using the outputs of the attention component as input, and is typically situated as the last step inside the Transformer block.
The intermediate representations that are processed by the feed-forward layer flow in a completely independent manner for each token inside the input sequence, therefore the execution of this block is parallelizable by nature.

Feed-forward blocks' principal contribution to the Transformer architecture consists in the introduction of nonlinearities, since the dot-product attention is still a predominantly linear process.
The feed-forward network acts as a \emph{grouping} mechanism, where information gathered in previous steps through attention is aggregated, processed and reformulated, adding depth to the computation.

The last Transformer block components are \textbf{Layer normalization} and \textbf{residual connections}, which represent techniques commonly found in most deep learning models and are generally utilized to improve gradient flow, training stability and overall generalization.
In particular:
\begin{itemize}
    \item \textbf{Layer normalization}~\cite{ba2016} is applied after each attention and feed-forward block.
It consists of two steps: a \emph{batch-independent normalization} step performed with parameters ($\mu$, $\sigma$) which model a normal distribution over the feature space of the vector representations, and a \emph{linear transformation} using a bias and a scale factor.
The bias and scale factor are usually referred to as $\gbm{\beta}$ and $\gbm{\gamma}$ respectively, and are trainable layer-level parameters which perform a \emph{shift and rescale} operation on the normalized vector.
...
In practice, residual connections sum the identity function of the input of a block to the output of that block, effectively creating a direct communication channel to which the block output is added.
In the Transformer's case the residual summation happens before layer normalization.
\end{itemize}
...
Given the fact that a Transformer model generates one token at a time, we would presumably obtain a single hidden representation at the end of the decoder stack, with a size compatible to that of the embedding space.
However, this is not a vocabulary token, and in order to output a vocabulary token we need the vector to lay in the vocabulary space.

To achieve this goal, the Transformer architecture features a last linear layer outside the main stack, which has the purpose of transform the last hidden representation from the embedding space to the vocabulary space, modeling an \emph{unembedding} operation.
In some specific model architectures, this last weight matrix is forced to be equal to the transpose of the initial embedding matrix for the sake of retaining a consistent representation between the first and last layers (this is known as \emph{weight tying}); other architectures opt to learn a different embedding representation space for the outputs of the model.

After the unembedding operation, the resulting vector of floating point numbers (commonly referred to as \emph{logits}) lays in the vocabulary space.
However, in order to get an actual probability distribution over the vocabulary, a last \emph{softmax} function also needs to be applied.
...
To describe current decoder architectures, our focus is primarily placed on the Llama 2~\cite{touvron2023} and Llama 3~\cite{dubey2024} (hereafter referred to simply as Llama) models, developed by Meta AI\footnotemark in 2023 and 2024 respectively.
...
As it is possible to observe from Llama's architectures (\cref{fig:background_llama-arch}), the first main novelty in new state-of-the-art models is the complete removal of the encoder from the architecture.
These models are often called ``decoder-only'', ``autoregressive'', ``causal'' or ``GPT-like'' from the \emph{Generative Pre-trained Transformer (GPT)} architecture~\cite{radford2018,radford2019,brown2020,openai2023} that popularized the use of Transformer models without the decoder, although the first documented use of a decoder-only architecture can be ascribed to this 2018 paper by~\citet{liu2018}.
...
The major improvement of autoregressive architectures consists in the massive reduction in parameters, which enables more flexibility during training, aside from the improvements tied to scalability and efficiency.
...
Another important change in the Transformer architecture is the \emph{Rotary Position Embeddings (RoPE)} mechanism, proposed in 2021 by~\citet{su2024}.
...
This feature is introduced due to one of the main flaws of absolute positional encoding being its inability to model dependencies between tokens, as each token encoding is independent of the others and, in particular, from the distance between tokens.

RoPE addresses this limitation by modeling both absolute and relative token positional information by encoding them as a \emph{geometric rotation} inside the embedding space.
...
The \emph{pre-normalization (pre-norm)}~\cite{baevski2019,xiong2020} approach consists in applying layer normalization directly inside the residual block, right before the attention and feed-forward blocks.
Additionally, a further layer normalization operation is performed before prediction, after the last Transformer block.
...
This improvement allowed the removal of the \emph{learning rate warm-up stage}: a technique that was commonly used in post-norm Transformer architectures to avoid diverging in training~\cite{popel2018} and involved a gradual increase of the learning rate in the first training epochs.
...
\begin{figure}[t!]
    ...
    \caption{Overview of the feed-forward neural network (FFNN) block implemented following the SwiGLU~\cite{shazeer2020} architecture.}
...
\begin{equation}
    ...
    FFNN(\gbm{x}) = GLU(\gbm{x}) \cdot \gbm{W}_{out}^{GLU} + \gbm{b}_{out} 
...
There are two principal methods to perform pre-training: \textbf{Masked Language Modeling (MLM)} and \textbf{Next Sentence Prediction (NSP)}.
\textbf{MLM} is popular between encoder architectures such as the BERT family, and consists of trying to infer certain \emph{masked} tokens given the surrounding ones by leveraging the provided context.
Whereas, \textbf{NSP} sees more use in the pre-training of GPT-like and decoder-based models by giving the model a pair of sentences and asking it to determine if the second sentence can logically follow the first one.
