...
These concepts are part of the \emph{transfer learning} paradigm, successfully applied to most deep neural architectures on a general scale.
The \todo{central point} of transfer learning is to avoid expensively re-training models over the same concepts by transferring general knowledge and specializing them on specific task separately. 
...
Nonetheless, one of the main concerns correlated with fine-tuning is the model's possibility of forgetting what was learned during pre-training.
This is mostly true for fine-tuning on the whole model, where all parameters are updated with the residuals obtained by training the model on the fine-tuning dataset.
An alternative to the costly task of fine-tuning a model from top to bottom is given by the possibility of performing the actual updates only on a subset of the parameters, resulting in a faster and less disruptive training.
This approach is commonly referred to as \emph{Parameter-Efficient Fine-Tuning (PEFT)} and includes various techniques that are used to reduce the amount of trained parameters, retaining most of the knowledge acquired during pre-training, while trying to introduce new concepts in a \todo{solid way}.
