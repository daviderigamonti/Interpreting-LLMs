...
\begin{equation}
    ...
    P(\ \cdot \mid \gbm{x}, d_\textit{dec}, n_\textit{norm}) = \operatorname{softmax}\Bigl(N_{n_\textit{norm}}(\gbm{x}) \cdot \gbm{W}_{d_\textit{dec}}\Bigr)
\end{equation}
...
\begin{subequations}
    \begin{align}
        ...
        \gbm{W}_\textit{max\_p} =\operatornamewithlimits{argmax}_{\gbm{W} \in \{\gbm{W}_\textit{in}^\T, \gbm{W}_\textit{out}\}} f(\gbm{W}) \label{eq:method_intravisto_max-p}
    \end{align}
\end{subequations}
...
Any of the matrices $\gbm{W}_\textit{in}^\T$, $\gbm{W}_\textit{out}$, $\gbm{W}_\textit{linear}$ and $\gbm{W}_\textit{quadratic}$ can be used as the decoder matrix $\gbm{W}_{d_\textit{dec}}$ to decode an embedding $\gbm{x}$ into a probability distribution over $\mathcal{V}$ as described in \cref{eq:method_intravisto_decoding}.
