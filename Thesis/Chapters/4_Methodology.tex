...
InTraVisTo is implemented entirely in \emph{Python}\footnote{\rlap{\url{https://www.python.org/}}}, utilizing the \emph{Plotly Dash}\footnote{\rlap{\url{https://dash.plotly.com/}}} framework as its core library to create an interactive, web-based graphical environment.
...
Whereas $\gbm{\gamma}$ and $\gbm{\beta}$ are the respective shift and rescale weights, corresponding to the last normalization layer present in the LM head of the model.
...
To do so, we compute the \emph{transition matrix} for our First Order Model (FOM) by combining the input and output weight matrices as follows:
\begin{equation}
...
\begin{equation}
    ...
    \begin{gathered}
        \gbm{Q}_{\textit{FOM}}^{RMS} = \operatorname{softmax}(\gbm{W}_{in,RMS} \cdot \gbm{W}_{out,RMS}) \\
        \text{where}\ \left\{
            \begin{array}{cl}
...
\end{equation}
Where $S$ denotes a sentence composed of a total of $T+1$ tokens identified as $S(0),\dots,S(T)$, and $(\gbm{Q})_{i,j}$ represents the indexing operation performed on the transition matrix $\gbm{Q}$ corresponding the transition probability from state $i$ to state $j$.
...
\begin{equation}
    ...
    \begin{gathered}
    {{\bar D}_{KL}}(\gbm{Q}_{ref} || \gbm{Q}_{model}) = \frac{1}{V}\sum_{v \in \mathcal{V}}{D_{KL}^v(\gbm{Q}_{ref} || \gbm{Q}_{model})} \\
    \text{where}\ D_{KL}^v(\gbm{Q}_{ref} || \gbm{Q}_{model}) = \sum_{w \in \mathcal{V}}{\bigl((\gbm{Q}_{ref})_{v,w}\bigr)\ln{\frac{(\gbm{Q}_{ref})_{v,w}}{(\gbm{Q}_{model})_{v,w}}}}
    \end{gathered}
\end{equation}
