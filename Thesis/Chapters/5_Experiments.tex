...
In \todo[orange]{top-k ...} we can notice that analogies of the type $w_1 - w_2 + w_4 = w_3$, seem to always return worst results than default ones: $w_2 - w_1 + w_3 = w_4$; this is likely the result of most analogies being created with a specific directionality in mind.
...
Although past work has theorized and partially demonstrated on a restricted set of models the possibility of this hypothesis being \todo{correct}~\cite{elhage2021}, we suspect that FOMs extracted from different LLMs may have different degrees of \todo{`Markovness'} and we strive to explore \todo{this} hypothesis in this section.
\todo[green]{Add part on weight tying}

Following the normalization intuition that \todo{proved successful} for InTraVisTo in \todo[orange]{ref section}, we also want to explore an alternative of the basic FOM obtained from directly joining input and output embeddings.
In fact, this alternative was introduced with the intention of circumventing one particular problem emerged during some experimental instances, which reflected similar complications already experienced and solved for the research question referenced in \todo[orange]{ref section research question}
The main issue can be described as an excessive \todo{`flatness'} of the vocabulary distribution for decoded states, in this case referenced to the probability distribution for the next token in the FOM transition matrix.
...
As we will establish in this section, the proposed solution does not obtain satisfying results \todo{as opposed} to its previous iteration.
...
From these models we choose to analyze LLaMA-2-7B \todo[orange]{cite}, Mistral-7B \todo[orange]{cite} and Phi-3-small-8k-instruct \todo[orange]{cite}.
LLaMA-3-7B \todo[orange]{cite} was also considered in addition to the other models.
However, its large vocabulary dimensionality (\todo[orange]{voc size}) entailed a \todo{size explosion} for the transition matrix.
Thus, \todo{it} was discarded due to computational unfeasibility reasons.
...
As previously mentioned in \todo[orange]{ref section}, we define a auxiliary class of FOMs which utilizes the RMS normalization in the transition matrix formulation.
The specific approach is formalized in \todo[orange]{ref method}, splitting the computation between input and output embeddings.

In addition, there is one important detail about the construction of the FOM transition matrix \todo{which} needs to be mentioned.
When the model performs a prediction, given how the unembedding matrix is structured, there needs to be a softmax operation to obtain a meaningful probability distribution, as already specified in \todo[orange]{ref section 2}.
Thus, in most scenarios, it is not possible to directly compare the FOM transition matrix against other transition matrices without \todo{carrying out} the conversion operation specified in  first.
As already explained in \todo[orange]{ref methodology}, the conversion from logit transition matrix to actual transition matrix is not performed on experiments that operate on \todo{aggregations of most likely results}.
...
Starting from this representation we adopt set metrics to compute similarities between sets of distributions (transition matrices) as defined in \todo[orange]{cref}.
...
On the other hand, by observing \todo[orange]{fig} depicting Phi 3.5's performance, we can observe that the general trend follows Llama 2's performance \todo{between} both top-$k$ tasks.
The only surprising behavior displayed by Phi is the exceptionally low accuracy in predicting the identity matrix as shown in \todo[orange]{fig}.
...
In fact, we can notice that all LLMs present the same trend regarding their matrix distance experiments, which is most likely caused by the noise present in their large matrices, making them unfit for direct comparison through a distance metric.
This observation confirms a potential bias towards lower distance estimates in the FOM/Markov case, possibly due to a smaller quantity of null values present in the Markov transition matrix \todo{with regard to} the identity matrix.
However, some slight differences between models emerge when considering the FOM variants that include RMS normalization.
In particular, \todo[orange]{table} shows that the FOMs with RMS obtained from Llama 2 and Mistral display a discrete bias towards the results found at the previous point concerning \todo[orange]{previous images}.
We can speculate that this percieved trend is able to emerge only due to the fact that RMS normalization aligns internal representations in a way that probabilities appear to be more \todo{`skewed'}, thus providing a better fit for their actual reference distribution.
...
Furthermore, the FOM with RMS obtained from Phi 3.5 is a clear outlier as its performance is significantly worse with respect to its counterpart without RMS than any other model, and \todo{for both} considered comparisons.
...
On the other hand, in \todo[orange]{table} we show the most likely token predicted by models for a restricted set of randomly chosen common input words.
This gives an approximate but immediate view on the kind of predictions that extracted FOMs and their respective trained Markov models tend to \todo{make}.
...
\todo[cyan]{overlap/sorensen/jaccard llama2}
\todo[cyan]{overlap llama2 FOM / FOM + RMS}

As mentioned before, we also introduce set metrics as a more reliable measure for transition matrix comparison.
Considering \todo[orange]{figure (1)}, it is possible to notice that the three chosen set metrics seem roughly equivalent at assessing comparison between models.
...
\todo[green]{maybe expand}
We also consider the effect of RMS normalization on FOMs in the context of this experiment: as it is possible to discern from \todo[orange]{figure (2)}, there does not seem to be any \todo{major} difference between the overlap coefficient of a FOM and its counterpart with RMS for Llama 2.
This is also the case for other models and metrics and can be attributed to the fact that, in this case, RMS only causes noticeable changes in the underlying probabily distribution, leaving the ordering of \todo{elements} mostly unaltered.

\todo[cyan]{overlap llama2}
\todo[cyan]{overlap mistral}
\todo[cyan]{overlap phi}
...
As we can observe in \todo[orange]{fig (1), fig (3)}, ignoring the raw accuracy, Llama 2 and Phi 3.5 present a high average overlap coefficient for low values of $k$, clearly outperforming both random baselines.
...
On the other hand, by observing \todo[orange]{fig (2)} we can Mistral's average overlap coefficient is just slightly above the baselines for low values of $k$.
...
By looking at Mistral's performance in \todo[orange]{fig (2)}, since the overlap coefficient corresponding to the FOM/Markov comparison \todo{dips} under the random baselines, we can infer that its FOM is not actually modeling noise, but a function that is completely separate from the next token prediction, \todo{namely self-regression}.
...
The KL divergence is computed according to \todo[orange]{ref} and, as already mentioned, is an asymmetric dissimilarity distance which measures how much a given distribution differs from \todo{another}.
In practice, given a token, we compute point-wise contributions for each vocabulary token and we aggregate \todo{them} through summation.
Then, we \todo{furtherly} aggregate the KL divergences obtained this way by averaging them, thus obtaining the average KL divergence over the entire vocabulary for each model.
...
By performing a preliminary analysis of the average perplexities in \todo[orange]{table 1} we can immediately notice that all models present similar trends in how their FOM performs with respect to the uniform baseline and the computed Markov model counterpart.
...
This last point is only marginally circumvented by the usage of a different test set, as by observing \todo[orange]{table 2} we can detect a meaningful raise in perplexity for Markov models, while still being significantly distant from the perplexity gathered by the FOMs.

Unsurprisingly we observe the perplexity results of the identity matrix predictions being disproportionately higher than all other models, as it \todo{represents} the certainty of all tokens repeating and is not fit to actually represent a meaningful Markov transition matrix.
However, we still value this outcome since it provides a \todo{ground of comparison} for the FOM extracted from each LLM, indeed confirming the results obtained in the previous experiment (\todo[orange]{ref}) implying that FOMs are generally closer to the actual Markov model rather than the identity matrix.
Nonetheless, we can speculate that the pattern unveiled in \todo[orange]{previous section} is still present in \todo[orange]{table 1, table 2} as the perplexity tied to the FOM based on Mistral is slightly higher than its LLama 2 and Phi 3.5 counterparts, possibly hinting at the fact that it might include similarities to the uniform and identity matrix distributions.

\todo[cyan]{perplexity openwebtext graph llama}
\todo[cyan]{perplexity openwebtext graph mistral}
\todo[cyan]{perplexity openwebtext graph phi}

One peculiar trend observable in \todo[orange]{image (1), (2), (3)} is the fact that, when plotting the perplexity over multiple sentences in a test dataset, a clear pattern with the shape of multiple `horizontal lines' emerges for the FOM version of most LLMs.
The lines vary in density and are placed in the loose proximity of the perplexity \todo{of the} uniform model.
...
For instance, the FOM based on Llama 2 aggregates the majority of sentences on the perplexity value that corresponds to the uniform baseline and a minority, \todo{albeit substantial}, of sentences on a consistently lower perplexity value than the uniform baseline.
...
\todo[cyan]{perplexity openwebtext graph mistral rms}
...
Even by comparing the gathered results with the previous experiment described in \todo[orange]{ref}, we can obvserve a severe underperformance of FOMs with RMS normalization as their average perplexity appears to be \todo{orders of magnitude} higher than both their standard FOM counterpart and the uniform baseline as noted in \todo[orange]{table 1, table 2}.
...
In addition, by observing \todo[orange]{perplexity owt mistral rms} we can see an interesting phenomenon consisting \todo{of} an apparent fragmentation of the two perplexity groups shown in \todo[orange]{perplexity owt mistral} into multiple groups with increasing perplexity.
\todo{Generally speaking}, it is possible that the worsened performance unveiled by the RMS normalization is indicative of actual model performance, as it removes the uniform correlation by skewing the probability distributions.
However, we \todo{deem} this conjecture as unlikely, since it seems to have different effects on models, in contrast with the findings shown in \todo[orange]{previous section}.

\todo{Conversely with the perplexity}, results obtained by computing the KL divergence metric over the vocabulary can be considered more straightforward to interpret as we can observe \todo{that there is} a clearer difference between the performance of models.
Interestingly, we notice that this series of outcomes is not necessarily in line with previous observations, however, we can ascribe discrepancies in the results to \todo{the direct} similarity between FOM probabilities and the uniform distribution.
Following this idea, we also experience interesting behaviors when considering the FOM variant which includes RMS normalization due to its direct effects on the \todo{uniformity} of FOM probabilities.

The distribution dissimilarity between FOM and Markov model is \todo{clearly} substantially lower than \todo{that of} any other pair of model distributions for all analyzed LLMs as shown in \todo[orange]{kl tables 1,2,3}.
...
On the other hand, the FOM possesses a much more uniform-like baseline as already shown in previous experiments (\todo[orange]{ref}), thus it \todo{acts better at being considered} the reference distribution in the KL divergence computation.
...
For instance, in \todo[orange]{kl table 2} we can notice a slightly higher $KL(Markov||FOM)$ \todo{formula} value and a slightly lower $KL(FOM||Markov)$ \todo{formula} value for the Mistral FOM compared against other models.
...
Firstly, in \todo[orange]{table 1} we can observe that the FOM based on Llama 2 performs slightly better than its alternate version including RMS.
...
However, if we also consider \todo[orange]{table 2} it is possible to notice that the performance drop tied to the FOM with RMS based on Mistral is actually much less noticeable than \todo{its} Llama 2 counterpart.
...
Conversely, \todo[orange]{table 3} shows the \todo{immense} impact on KL divergence that RMS normalization has on the FOM based \todo{specifically} on Phi 3.5, resulting in a significant decrease of performance across all comparisons that include the FOM and reflecting the results of \todo{the} previous experiment on perplexity.
