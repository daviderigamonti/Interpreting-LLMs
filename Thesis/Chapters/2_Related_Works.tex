...
\citet{rai2024} propose a taxonomy for interpretability techniques centered aroung the concept of Mechanistic interpretability (MI).
...
Another possible taxonomy for interpretability techniques, more focused on their nature rather than their use, is presented \citet{ferrando2024}.
...
\textbf{Causal interventions} approaches are centered around the interpretation of the LM as a causal model \cite{geiger2021}, which takes the form of a directed acyclic graph (DAG) having model computations as nodes and activations as edges.
...
Most of the initial work regarding transformer circuits was performed on publications belonging to the `transformer circuits thread' \todo[orange]{cite}, heavily inspired by the precurring vision counterpart \cite{cammarata2020}.
...
The central idea for this hypothesis is based upon the early discoverings of linearity inside the embedding space done by \citet{mikolov2013}, as the resulting analogies and geometric properties are direct consequence of a linear embedding space.
Recent work has uncovered many instances of FFN neurons that consistently fire with specific patterns \todo{traceable} to input features \cite{voita2023}, suggesting that this behavior is an effect of the next token prediction training paradigm \cite{jiang2024}.
