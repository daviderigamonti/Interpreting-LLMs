...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Concatenating multi heads outputs TODO
        if self.config.pretraining_tp > 1:
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # pop debugging vectors
        input_residual_embedding = kwargs.get('input_residual_embedding')
        post_attention_embedding = kwargs.get('post_attention_embedding')
        attention_plus_residual_embedding = kwargs.get('attention_plus_residual_embedding')
        post_FF_embedding = kwargs.get('post_FF_embedding')



        residual = hidden_states

        # Debug embedding vector
        input_residual_embedding.append((layer_id, hidden_states))
        ...
        # Debug embedding vector
        post_attention_embedding.append((layer_id, hidden_states))

        hidden_states = residual + hidden_states
        
        # Debug embedding vector
        attention_plus_residual_embedding.append((layer_id, hidden_states))      
        ...
        # Debug embedding vector
        post_FF_embedding.append((layer_id, hidden_states))
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # Declare debugging supporting vectors
        self.input_residual_embedding = []
        self.attention_plus_residual_embedding = []
        self.post_attention_embedding = []
        self.post_FF_embedding = []
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # TODO pass idx to decoder layer!!!!!!!!!!!!!!!!!!!!!
                layer_outputs = decoder_layer(
                    ...
                    use_cache=use_cache,
                    layer_id=idx,
                    input_residual_embedding=self.input_residual_embedding,
                    attention_plus_residual_embedding=self.attention_plus_residual_embedding ,
                    post_attention_embedding=self.post_attention_embedding,
                    post_FF_embedding=self.post_FF_embedding
                )
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def _apply_interpolated_embedding(self, hidden_states):
        """
        """
        pred_ids = []
        per_token_logits = []
        for layer_n, layer in enumerate(hidden_states):
            input_logits = torch.matmul(layer.to(self.model.embed_tokens.weight.device), self.model.embed_tokens.weight.T)
            output_logits = self.lm_head(layer)
            output_logits = output_logits.float()
            interpolated_embedding = ((33 - layer_n) * (input_logits) + (layer_n) * (output_logits)) / 33
        
            probabilities = torch.nn.functional.softmax(interpolated_embedding.squeeze(), dim=0)
            token_id = torch.argmax(probabilities)  

            prob = round(float(probabilities[token_id]) * 100, 2)
            pred_ids.append(token_id)
            per_token_logits.append(prob)
        return pred_ids, per_token_logits
    ...
    def get_hidden_states_predictions(self, embedding="output"):
        ...
        for n_token in range(1, len(self.model.hidden_states)):
            if embedding == 'output':
                ...
                pred_ids, per_token_logits = self._apply_interpolated_embedding(self.model.hidden_states[n_token])
