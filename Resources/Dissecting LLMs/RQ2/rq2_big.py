...
# Logging
setup_logger(LOG_PATH, LOGGER_NAME, "markovbig", logging.DEBUG)
...
try:
    ...
    if not os.path.exists(dump_path + "ftm.pt"):
        fom_trans_matrix = torch.matmul(
            fom_model["input"].weight.data, fom_model["output"].weight.data.T
        )
        # torch.save(fom_trans_matrix, dump_path + "ftm.pt")
    else:
        fom_trans_matrix = torch.load(dump_path + "ftm.pt")
    ...
    graph_test_sim(overlap_metrics, n_k, limit=5, xlabel="k", ylabel="Similarity", title="Average set similarity metrics between vocabulary words predicted by FOM/Markov model", image_name=f"{image_path}overlapmetrics5.pdf")
    graph_test_sim(overlap_metrics, n_k, xlabel="k", ylabel="Similarity", title="Average set similarity metrics between vocabulary words predicted by FOM/Markov model", image_name=f"{image_path}overlapmetrics.pdf")
    ...
    del perp_markov, perp_fom, perp_I, perp_random, perp_metrics
    markov_trans_matrix = torch.load(dump_path + "mtm.pt")
    gc.collect()
    torch.cuda.empty_cache()
    ...
    if os.path.exists(owtperp_dump):
        ...
        markov_trans_matrix = markov_trans_matrix.log().cpu()
        perp_markov = compute_perplexity(owt, tokenizer, lambda tok: markov_trans_matrix[tok, :]).cpu()
        ...
        del fom_trans_matrix
        I = torch.eye(n_voc).cpu()
        gc.collect()
        torch.cuda.empty_cache()
        
        I = I.add_(1e-12).log_().cpu()
        perp_I = compute_perplexity(owt, tokenizer, lambda tok: I[tok, :]).cpu()

        del I
        gc.collect()
        torch.cuda.empty_cache()

        logrand = torch.full((n_voc,), 1 / n_voc).log().cpu()
    ...
    del perp_markov, perp_fom, perp_I, perp_random, perp_metrics
    fom_trans_matrix = torch.nn.functional.log_softmax(torch.load(dump_path + "ftm.pt"), dim=-1).cpu()
    markov_trans_matrix = torch.load(dump_path + "mtm.pt").log().cpu()
    gc.collect()
    torch.cuda.empty_cache()
    ...
    kl_fom_markov = kl(fom_trans_matrix, markov_trans_matrix)
    kl_markov_fom = kl(markov_trans_matrix, fom_trans_matrix)
    ...
    kl_fom_I = kl(fom_trans_matrix, I)
    kl_I_fom = kl(I, fom_trans_matrix)
    ...
    kl_markov_I = kl(markov_trans_matrix, I)
    kl_I_markov = kl(I, markov_trans_matrix)
